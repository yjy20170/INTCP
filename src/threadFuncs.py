...
def funcLinkUpdate(mn,netParam):
    ...
    def config(intf,new_bw,loss):
        #intf.node.monitor(timeoutms=10)
        intf.cmd('echo')
        intf.config(bw=new_bw,delay=str(netParam.rtt/4)+"ms",loss=loss)
           
    while not Thread.stopped():
        time.sleep(5)
        new_bw = generate_bw(netParam.bw,netParam.varBw,1,"random")

        for intf in s2.connectionsTo(pep)[0]:
            atomic(config, intf,new_bw,0)
        for intf in s2.connectionsTo(h2)[0]:
            atomic(config, intf,new_bw,netParam.loss)
...
def funcIperfPep(mn,netParam):
    ...
    for i in range(5):
        print("iperfc loop %d starting, sendTime = %d" %(i,netParam.sendTime))
